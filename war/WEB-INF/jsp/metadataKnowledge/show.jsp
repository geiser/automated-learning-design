...
<html>
<head>
	<title>Knowledge : <c:out value="${metadata.identifier}" /></title>
			...
			if (relationCount < 10) {
				...
				$("#relationContainer").append("" +
					"<div id='" + relationID + "'>" +
				    	...
				    	"</select>*" +
				    	"&nbsp;&nbsp;&nbsp;&nbsp;" +
						"<label for='relations[" + relationCount + "].value.id'>Knowledge</label>" +				
						"<select class='required' id='relations[" + relationCount + "].value.id' name='relations[" + relationCount + "].value.id'>" +
				    		...
				    		"<option value='<c:out value="${knowledge.id}" />'><c:out value="${knowledge.propertyMap['title'][0].value}" /></option>" +
					    	</c:forEach>
				    	"</select>*" +
					"</div>");
				relationCount++;
...
<body>
    ...
    <h1>Knowledge : <c:out value="${metadata.identifier}" /></h1>
    
    <form class="cmxform" id="form" name="dados" method="POST" action="<c:url value="/metadatas/knowledges" />" accept-charset="UTF-8">
    <fieldset id="knowledge">
    	...
    	<p>
    		...
    		<input id="metadata.type" type="hidden" name="metadata.type" value="knowledge" />
...
<!--					    		<option value="isVariantOf" <c:if test = "${relation.name eq 'isVariantOf'}">selected="selected"</c:if>>isVariantOf</option>-->
							...
							<select class='required' id='relations[<c:out value="${relationStatus.count-1}" />].value.id' name='relations[<c:out value="${relationStatus.count-1}" />].value.id'>
								<option value=''>-- choose value --</option>
					    		<c:forEach varStatus="knowledgeStatus" var="knowledge" items="${knowledges}">
					    		<option value="<c:out value="${knowledge.id}" />" <c:if test = "${knowledge.id eq relation.value.id}">selected="selected"</c:if>><c:out value="${knowledge.propertyMap['title'][0].value}" /></option>
						    	</c:forEach>
					    	</select>*
    ...
    <c:if test="${not empty (metadata.id)}" >
    <form id="formRemove" name="dados" method="POST" action="<c:url value="/metadatas/knowledges/${metadata.id}" />" accept-charset="UTF-8">
